...
/**
 * MQTT v3.1.1 §2.2.3 / v5 §2.1.4 Remaining Length encoding (7 data bits + continuation
 * bit per byte, LSB-first, max 4 bytes).
 *
 * Implements [BoundingLengthCodec] so the buffer-codec processor wraps subsequent fields
 * in the outer-limit-capture try/finally pattern, driven by interface inspection from the
 * `@FramedBy(MqttRemainingLengthCodec::class, after = "header")` annotation on the
 * `ControlPacketV4` / `ControlPacketV5` sealed parents.
 */
object MqttRemainingLengthCodec : BoundingLengthCodec<UInt> {
    ...
    override fun decode(
        ...
        throw DecodeException(
            ...
            actual = "5th continuation byte (malformed per MQTT §2.2.3 / §2.1.4)",
        )
    }
