...
/**
 * Non-bounding VBI codec for v5 SubscriptionIdentifier (§2.2.2.2 + §3.8.2.1.2).
 * Same wire shape as [MqttRemainingLengthCodec] (7-bit-per-byte continuation,
 * LSB-first, max 4 bytes), but implements plain [Codec] instead of
 * `BoundingLengthCodec` — the decoded value is just a typed `UInt` body and
 * does not narrow the buffer for subsequent fields.
 */
