...
/**
 * Deterministic fuzzing of the v5 control-packet decode path. Targets
 * [decodeProductionV5] — the same `decodeAggregating` boundary production runs via
...
class ControlPacketV5FuzzTest {
    ...
    fun randomBytesDecodeSafely() {
        repeat(10_000) { i ->
            ...
            if (bytes.isNotEmpty() && rng.nextBoolean()) {
                bytes[0] = ((1 + rng.nextInt(15)) shl 4 or rng.nextInt(16)).toByte()
            }
            fuzzCase(i, seed, bytes) {
                decodeProductionV5(bytesToReadBuffer(bytes))
            }
        }
    }
    ...
    fun mutatedValidPacketsDecodeSafely() {
        repeat(2_000) { i ->
            ...
            val rng = Random(seed)
            val encoded = encodeToReadBufferV5(randomValidV5Packet(rng))
            val mutated = mutate(encoded.readByteArray(encoded.remaining()), rng)
            fuzzCase(i, seed, mutated) {
                decodeProductionV5(bytesToReadBuffer(mutated))
            }
        }
    }
    ...
    fun roundTripRandomValidPackets() {
        repeat(2_000) { i ->
            ...
            val rng = Random(seed)
            val packet = randomValidV5Packet(rng)
            val encoded = encodeToReadBufferV5(packet)
            val bytes = encoded.readByteArray(encoded.remaining())
            fuzzCase(i, seed, bytes) {
                assertV5PacketsEqual(packet, decodeV5(bytesToReadBuffer(bytes)))
            }
        }
    }
}
