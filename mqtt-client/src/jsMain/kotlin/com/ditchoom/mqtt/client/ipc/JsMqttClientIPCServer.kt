...
class JsMqttClientIPCServer(private val delegate: MqttClientIPCServer, private val port: MessagePort) {
                    ...
                    when (messageType) {
                        MESSAGE_TYPE_CLIENT_PUBLISH -> {
                            delegate.scope.launch {
                                ...
                                port.postMessage(buildSimpleMessage(MESSAGE_TYPE_CLIENT_PUBLISH_COMPLETION, messageId as Int))
                            }
                        }
                        MESSAGE_TYPE_CLIENT_SUBSCRIBE -> {
                            delegate.scope.launch {
                                delegate.onSubscribeQueued(packetId)
                                port.postMessage(buildSimpleMessage(MESSAGE_TYPE_CLIENT_SUBSCRIBE_COMPLETION, messageId as Int))
                            }
                        }
                        MESSAGE_TYPE_CLIENT_UNSUBSCRIBE -> {
                            delegate.scope.launch {
                                delegate.onUnsubscribeQueued(packetId)
                                port.postMessage(buildSimpleMessage(MESSAGE_TYPE_CLIENT_UNSUBSCRIBE_COMPLETION, messageId as Int))
                            }
                        }
                        ...
                        MESSAGE_TYPE_CLIENT_CONNACK_REQUEST -> {
                            delegate.scope.launch {
                                val connack = delegate.currentConnack()?.serialize(AllocationZone.SharedMemory) as? JsBuffer
                                port.postMessage(
                        ...
                        MESSAGE_TYPE_CLIENT_AWAIT_CONNECTIVITY_REQUEST -> {
                            delegate.scope.launch {
                                val connack = delegate.awaitConnectivity().serialize(AllocationZone.SharedMemory) as JsBuffer
                                port.postMessage(
